...
const CustomerInfo = ({ label, value }: { label: string, value: string }) => (
...
const Receipt: React.FC<ReceiptComponentProps> = ({ data }) => {
  const { customer, summary, medications, pharmacy } = data;
    ...
    <div className="w-full max-w-3xl mx-auto receipt-paper p-6 rounded-lg animate-fade-in">
      ...
      <div className="mb-6 p-4 bg-pharmacy-lightBlue/30 rounded-md animate-scale-in">
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          {customer.date && (
            <CustomerInfo label="التاريخ:" value={formatDate(customer.date)} />
          )}
        </div>
      </div>
      ...
      <div className="mt-6 text-center no-print animate-fade-in">
        <button
          ...
          className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-pharmacy-coral hover:bg-pharmacy-coral/90 focus:outline-none focus:ring-2[...]
        >
          <svg 
